#!/usr/bin/env python3
"""
update_cumulative_inflation.py  (v3 — correct BLS series IDs)

Key fix: CPIAUCSL is a FRED ID. The correct BLS API ID is CUSR0000SA0.
Boston city-level series (CUURA103SA0) require a BLS API key.
Get a free key at: https://data.bls.gov/registrationEngine/
Add it as a GitHub repo secret named BLS_API_KEY.
Until then, Boston arrays will be null (charts will show gaps).
"""
...
# Correct BLS API series IDs (NOT FRED IDs)
# CUSR0000SA0  = National CPI-U, All Items, Seasonally Adjusted
# CUURA103SA0  = Boston CPI-U, All Items (requires BLS API key)
# CUSR0000SAH  = National Shelter, Seasonally Adjusted  (no key needed)
# CUURA103SAH  = Boston Shelter (requires BLS API key)
# CES0500000003 = Avg Hourly Earnings, Total Private (no key needed)
SERIES_IDS = {
    "nat_cpi":    "CUSR0000SA0",    # FIXED: was CPIAUCSL (FRED ID)
    "bos_cpi":    "CUURA103SA0",    # needs BLS API key
    "nat_shelter":"CUSR0000SAH",
    "bos_shelter":"CUURA103SAH",    # needs BLS API key
    "wages":      "CES0500000003",
}
...
def fetch_bls(series_list, start_year=2020, end_year=None):
    ...
    if api_key:
        print("BLS API key found — city-level series (Boston) will be available.")
    else:
        print("No BLS API key — Boston series will be skipped. "
              "Get a free key at https://data.bls.gov/registrationEngine/ "
              "and add it as a repo secret named BLS_API_KEY.")
...
def parse_series(bls_response, series_id):
    for series in bls_response.get("Results", {}).get("series", []):
        ...
        if result:
            ...
            print(f"  {series_id}: no data (may need API key for city-level series)")
        return result
    return {}
...
def replace_var(html, varname, new_literal):
    ...
    if count == 0:
        print(f"WARNING: 'var {varname}' not found in HTML — skipped")
    return new_html
